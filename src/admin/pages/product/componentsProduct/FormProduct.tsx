...
import type { Product, Size } from "@/shopFrontend/types/product.type";
...
export function FormProduct({product, productSubtitle, productTitle, isDisabled, onSubmit}: PropsFormProduct){
  ...
  const inputRefTag = useRef<HTMLInputElement | null>(null);
  const inputFileRef = useRef<HTMLInputElement | null>(null)

const selectSizes = watch("sizes");
const selectedTags = watch("tags");
const selectedImges = watch("files");

 const addTag = () => {

    if(inputRefTag.current && inputRefTag.current.value !== ""){

      const inputValue = inputRefTag.current.value;
      const tagsAdded = new Set(getValues("tags"));
      tagsAdded.add(inputValue);
      setValue("tags", Array.from(tagsAdded));
  ...
  const removeTags = (tagRemove: string) => {

    const newTags = selectedTags.filter((tag) => tag !== tagRemove);
  ...
  const removeSize = (sizeToRemove: string) => {
    const deleteSizes = selectSizes.filter((s) => s !== sizeToRemove);
    setValue("sizes", deleteSizes);
  };
  ...
  const removeImageSelected = (nameImg: string) => {
    const currentFiles = getValues("files");
    if(!currentFiles) return;
    
    const newImgs = currentFiles.filter((img) => img.name !== nameImg);
    setValue("files", newImgs);
  ...
  return (
    <form onSubmit={handleSubmit((data) => {
      const productWithcFiles = {
        ...data,
        files: getValues("files")
      }
      onSubmit(productWithcFiles);
    })}>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    />
                    {errors.price && <p className="text-red-500">Error en el precio</p>}
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-200">
                  ...
                  {availableSizes.map((size) => {
                    ...
                    const isSelected = selectSizes?.includes(size);
            ...
            <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-6">
              ...
              <div className={cn('mt-6 space-y-3', {
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    selectedImges?.map((img, index) => (
                      <div className="relative group" key={index}>
                        ...
                        <button
                         ...
                         onClick={() => removeImageSelected(img.name)}
                         >
                          <X className="h-3 w-3" />
                        </button>
                      </div>
                  ...
                  {
                      selectedImges?.length === 0 && <p className="underline-offset-1">No hay archivos seleccionados</p> 
                  }
                </div>
              </div>
            </div>
